...
//public mode
    amazone.addCommand({pattern: 'vid ?(.*)', fromMe: KSK, desc: YTV_DESC}, (async (message, match) => { 
...
//in public for me 
 ...
 amazone.addCommand({pattern: 'vid ?(.*)', fromMe: PUBH, desc: YTV_DESC}, (async (message, match) => { 
...
const tk = Config.WORKTYPE == 'public' ? false : true
    ...
    amazone.addCommand({ pattern: 'vid ?(.*)', fromMe: tk, deleteCommand: false, desc: Lang.MP4,  deleteCommand: false}, async (message, match) => {
        const linkk = match[1]
        if (!linkk) return await message.client.sendMessage(message.jid,YT_NEED,MessageType.text)
